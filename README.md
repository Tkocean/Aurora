...
## Manual Install:
- Step 1: Download the source code package from the [Source](https://codeload.github.com/Tkocean/Aurora/zip/refs/heads/main) page and extract it. Move the main program `Aurora` from the `/system/bin/` directory inside the extracted folder into the system variable, e.g. `/data/adb/ksu/bin`.
- Step 2: Manually create the directory `/data/adb/Aurora` and move the `Config` folder from the source code package into this directory.
